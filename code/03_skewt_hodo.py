...
with open(OUTPUT_REPORT_FILENAME, 'w') as report_file:
    ...
    for fp_index in range(len(ds_squeezed.forecast_period)):
        ...
        fig.suptitle(f"{title_line1}\n{title_line2}", fontsize=14, y=0.98)

        skew = SkewT(fig, rotation=45, rect=gs[0].get_position(fig))
        ...
        cbar = fig.colorbar(line, cax=cax, label='Height (km AGL)')
        hodo_ax.set_xlabel('U-Wind (m/s)'); hodo_ax.set_ylabel('V-Wind (m/s)')

        valid_time_str_file = valid_time_pd.strftime('%Y%m%d_%Hz')
        output_plot_filename = f"skewt_hodograph_kemayoran_jakarta_initial_{initial_time_str_file}_forecast_{valid_time_str_file}.png"
